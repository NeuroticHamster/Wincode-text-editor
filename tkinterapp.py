...
class controlers:
	...
	def searched(widget=None, editor=None, butts=None, linelabel=None):
		...
		#---------------------testing fstrings. guess thats new to python----
		'''a = 'girl'
		news = 'this new shit is weird'
		b = 'guy'
		print('newstring')
		h = f'(*counts)'
		print(h)
		#h='check formating {}'.format('hi')
		#c.execute('select (%s) from table), ())
		translatestring = str(counts).replace(')', '')
		translatestring = translatestring.replace('(', '')
		h = 'girl'
		print(f'{h}')
		testing = 'checkformating {}'.format('test')
		print(testing)'''
		
		translatestring = str(counts)[1:-1]
		linelabel['text'] = f'{translatestring}'
